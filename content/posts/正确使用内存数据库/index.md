...
另外，针对 Redis DML 的不可靠性，官方提出了 [RedLock](https://redis.io/docs/latest/develop/use/patterns/distributed-locks/#the-redlock-algorithm) 作为解决方案。该算法的核心思路是通过 [仲裁（Quorum）](https://zh.m.wikipedia.org/zh-cn/Quorum_(%E5%88%86%E5%B8%83%E5%BC%8F%E7%B3%BB%E7%BB%9F)) 来实现（CAP）一致性。但鉴于实施成本问题（至少要 3 个 Redis 实例，最好是 5 个），个人建议还是选择更加可靠的解决方案。例如 Zookeeper（ZAB CP），或者 Etcd（Raft CP）。
