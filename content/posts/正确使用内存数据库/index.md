...
Redis 提供了两种持久化方案：（1）RDB (Redis Database)，条件间隔全量快照。AOF (Append Only File)，增量指令备份。
...
注意，以上问题即便采用混合持久化方案（RDB + AOF）和复制架构也无法解决。因为 Redis 本身就不是为可靠存储而设计的。其实墨非定律已经道明一切，所以切勿存在侥幸的心理。

💡 建议
- 当需求无法接受数据丢失时，就不应该只将数据存储在 Redis 中
- 必要时，可以考虑将*应用程序日志*纳入为补偿服务的依据。但敏感信息应该先进行脱敏；以防数据丢失后彻底无法恢复
