...
另外，光靠*嵌入*和*引用*两种模式可能无法满足所有需求场景。譬如在社交网络中，明星的追随者（followers）数量通常是巨大的（可能几千万，甚至数亿）。此时即便只是将追随者标识嵌入到文档中，也无法避免文档变大。但这里的问题关键在于，一般的查询用例其实并不需要获取追随者的信息。那么此时应该考虑将追随者独立成一个集合[^2]，而不是继续采用*嵌入*或*引用*来实现关联。这种模式在 SQL 中称为*垂直切分*。
...
一个较为典型的反模式是将 SoR 的*数据模型*直接映射到*派生数据库*[^3]。
这种做法会一定程度丢失派生数据库的优势；特别是数据库类型为 NoSQL 时，因为 NoSQL 通常不具备完整的*ACID事务*特性。

[^1]: 采用嵌入式数据建模，会一定程度降低数据一致性。这一点需要紧记。
[^2]: 追随者集合的模式可以是 `{"_id": 明星标识, "followers":[追随者标识]}`
[^3]: 建议只有在 SoR 不能满足实际需求时才应该引入*派生数据库*。因为同时维护多个数据库会一定程度降低数据的一致性，同时还会带来额外的维护和设计成本。
