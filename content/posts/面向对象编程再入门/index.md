...
| ----------- | ----------- |
| 不变性 | 在程序生命周期内需要恒成立的条件。 |
| 对象 | 被观察的事物、（OOP）类的具体实例。|
| 行为 | 对象对外承担的职责。|
| 实体 | 具有业务价值和生命周期管理的对象。|
| 概念模型 | 对问题空间进行高度抽象和形式化后的产物，与实现细节无关。目的是期望进一步理清问题空间，以便更正确地解决问题。通常的解决方案是基于实体和各种示意图来达到该目的。值得注意，概念模型有时被称作*业务模型*。具体可以参考《分析模式》一书 |
| 反模式 | 在实践中经常出现，但违背良好设计原则的解决方案。|
| 贫血（领域）模型 | 一种 OOP 反模式，常见于数据驱动开发。特定是实体仅作为数据载体，而不具有业务性质的行为。因为违反了封装原则，所以严格上来说贫血模型并不是 OOP，而是面向数据编程（DOP）。 |
| 心智模型 | 人类对某一事物运作方式的主观理解和认知。|
| 虚方法 | 能被重写的实例方法。例如被 默认修饰符、public 或 protected 所修饰的方法。|
| 应用程序接口/API | 泛指程序与程序之间的连接点（而非指代 Java 的 interface 特性）。对于 OOP 而言，对象的 API 实际上就是行为（即对外公开的方法）。|
| 信息隐藏 | 将“相较稳定的部份”和“容易变化的部份”隔离，避免在变化时需要大面积改动设计或代码。要想做到这点，需要设计一个相较稳定的 API，而且该 API 应该能够将那些无需被外界（通常是客户端）感知的信息隐藏起来。|


## 面向对象工具

概念模型只能作为相关人员理解问题空间的工具，它本身并不具备直接映射为编程语言的条件（抽象程度高，缺乏细节信息）。所以，实际上设计人员应该先探索概念模型，然后再结合需求对实体作进一步设计。

面向对象分析（OOA）本质上是一种概念模型建模工具。核心理念是将问题空间建模成一系列相互作用的对象集合。

面向对象设计（OOD）是将概念模型转化为一个可行的实现方案。该工作需要站在业务领域和系统角度对对象作进一步探索和设计。相关细节信息（属性和行为）也在该环节进行补充。

面向对象编程（OOP）负责将 OOD 的产出映射为计算机可执行程序。OOP 会以类（Class）的形式来定义实体（一个包含了实体静态定义的程序单元），然后再以对象（Object）的形式在内存中执行计算。注意，如果映射目标只是一个简单的过程且没有与之对应的概念（类）时，可以将其构建成函数（在  Java 中可以用静态方法实现[^1]）。
