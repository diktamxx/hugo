...
对于该函数而言，前置条件是，参数 username、password、email_address 都是必须的（因为没有任何表达“可选”语义的迹象），而且都是 str 数据类型。另外，email_address 还需要符合邮箱地址的规格。而后置条件是，在注册成功后返回对应的用户标识，它是一个 str 数据类型。而不变性则是，用户名发生重复时会抛出一个 ValueError 异常。
