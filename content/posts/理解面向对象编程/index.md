---
title: "理解面向对象编程（Object-oriented programming，OOP）"
...
| 行为 | 对象对外承担的职责。|
| 实体 | 在唯心主义哲学中可理解为“能被意识到的东西”。而在编程领域中，实体通常是指具有唯一性的对象。|
| 概念模型 | 对问题域进行高度抽象和形式化后的产物。这样做的目的是期望进一步理清问题域本身的底层逻辑，以便更正确地解决问题。换句话说，对问题域进行概念建模有利于更好地理解和传达信息。譬如在软件工程中存在着一个根本性问题（域）是<u>需要有一种高度抽象的概念来形式化现实问题</u>，而该问题通常的解决方案是基于（哲学）实体和各种示意图来达到抽象和形式化的目的。值得注意，在软件分析设计中，概念模型有时会被叫作业务模型或领域模型。它们本质上是同一个东西，与实现细节无关。|
...
概念模型只能让相关人员更好地理解问题域，它本身并不具备直接映射为编程语言的条件。因为抽象性高，缺乏了大部份的细节信息。换言之，设计人员首先需要探索概念模型，并结合在该过程中形成的心智模型来对实体进行进一步设计。

在 OOP 中，会以 Class 形式对实体进行进一步设计。
Class 是一个包含了实体所有细节信息（状态和行为）的程序单元。从程序角度而言，其实 Class 相当于是对实体的进一步抽象。因为真正具体化实体概念的是 Object。而 Object 是 Class 的（其中一个）具体实例，它代表一个可独立执行计算的实体。

### 三大特性回顾

Class 必须将实体的状态[^1]和行为捆绑在一起，然后对其状态施加访问控制。
也就说，实体的状态只能透过行为来变更，而不应该被直接访问和修改[^2]。
该特性称为**封装**；是 OOP 最重要，但同时又是最容易被忽视的一个。封装的目的是希望通过行为来保护不变性；以免规则被有意或无意地绕过，从而导致实体内部状态被破坏。
...
**多态**本质上是一种允许程序在运行时才确定具体行为的机制。甚至可以这么说，OOP 之所以流行的一个重要原因就是多态。因为基于多态这一特性，开发人员可以很容易就编写出可测试和可灵活扩展的代码。这对于本身比较复杂且又需要长期迭代的系统来说很有帮助。从原理上来说，运行时多态是透过虚方法调用机制来实现的（该方式有时也被称为“动态分派”）。每个对象在内存中都有一个所属类的虚方法表引用。虚方法表记录了方法签名与具体方法体的关系，而这种关系只有在子类重写了父类方法时才会改变。所以当一个对象的虚方法并执行时，其实与引用该对象的变量类型没有任何关系。
...
[^1]: Object 的状态大致可以分为两种：数据属性、计算属性。前者会被持久化存储；后者则基于前者和特定规则计算所得。拓展：实践备忘录模式（也叫快照模式）[GoF]时，只需关注数据属性。
