...
## 线程种类
- 平台线程：泛指 JVM 线程（如：java.lang.Thread），负责执行用户态代码（如：应用程序任务）。在 64bit 系统中每条线程默认占 1MB（参考：`java -XX:+PrintFlagsFinal --version | grep ThreadStackSize`）；平台线程本质上只是 JVM 中的一个逻辑对象。没有能力直接向 CPU 申请计算资源，也没有能力直接操作内存页表，这些工作都需要由内核线程来实现。这就是为什么平台线程的使用成本较高，且经常见到它被池化复用的原因。
- 内核线程：由操作系统管理，负责底层资源访问。在 HotSpot 中，平台线程和内核线程是一对一的关系。也可以将内核线程视为平台线程的载体。
...
I/O 模型和线程的关系
- 曾经有个著名的问题叫 [C10k](https://en.wikipedia.org/wiki/C10k_problem) —— 让服务器能够同时处理 10000 个客户端请求。但以前的 I/O 操作属于同步阻塞式。这通常意味着请求数有多大线程数就要多大。否则服务器会被阻塞，从而无法处理后续请求；严重起来甚至会崩溃。所以对于旧时的服务器配置来说，处理该问题相当有难度。直到异步 I/O 模型的出现（准确来说是多路复用机制。它能够同时监听多个文件描述符），单条线程终于可以同时处理多个 I/O 而不会被一直阻塞，只需少量线程就能够处理大并发需求 —— 著名的 EventLoop 模型就此诞。或者你可能听说过 Reactor 模式（如：Netty）和 Node.js，其核心都是 EventLoop 模型/线程。通过线程独占资源来避免并发性（即无需 synchronized 或 volatile），再透过开发规范来约束开发者切勿阻塞 EventLoop 线程来达到 CPU 利用率最大化（，否则整个 EventLoop 处理都会被阻塞）。这就是 EventLoop 高效的根本原因 —— 高吞吐、低资源消耗。另外，如果纯粹将 EventLoop 模型和虚拟线程进行对比，前者的优势将不再明显（，但两者本身完全可以通过结合来使用）。之所以提出这种对比，是希望在技术选型或者迁移时能够作为参考。

协程和虚拟线程的区别
- 协程是一种语言层面，能够用于编排异步任务执行的高级 API。一般由编译器和运行时库实现 —— 底层可能是 EventLoop 模型。虚拟线程则是一种 JVM（用户态）层面的异步任务调度机制 —— 偏底层，对于客户端来说几乎透明。两者都支持结构化并发。但前者通常是在语法层面支持，而后者则通过 API 支持。
