...
理解例子：用户`a`、`b`共用一个电子钱包（假设余额为100元）。他们在相互不知情的情况下，几乎同时进行消费（假设`a`消费了70元，而`b`消费了50元）。在该场景下，事务操作就会出现**并发修改**问题，从而导致业务规则被破坏。例如：实际消费超出了余额。注意，“超出”并不一定体现为余额小于0，它也可以是**更新覆盖**。譬如`a`消费了70元后余额为30元，`b`消费50元后余额为50元。但因为`a`消费稍微比`b`要快一些，所以最终余额就会是50元，而非30元。因为`b`的更新覆盖了`a`的更新。之所以发生此类问题，其**根本原因是其操作不具备原子性和隔离性**。因为“消费”这一动作用程序来实现其实需要多个步骤：(1)读取余额，然后(2)从余额中减去消费额度，最后(3)保存计算结果。从并发编程角度来说，“先读后写”的任务应该具原子性和（资源访问）互斥性。其中原子性可以由本地事务或分布式事务来提供支持。而互斥性则可以在“读取余额”操作上施加*排他锁（如：for update）* 或是在“保存计算结果”操作上施加*乐观锁（如：CAS + Version）* 来实现。另外，从数据定义层面而言，余额之所以能够小于0是因为余额字段不是无符号类型。应该根据业务规则来选择合适的数据类型。
