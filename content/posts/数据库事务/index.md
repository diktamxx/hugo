...
<b style="color:#2C5B66">一致性（Consistency）</b>在不同的上下文中会有不同的解释。在数据库事务上下文中，可将该承诺理解为<u>确保数据的相关*约束规则*免遭破坏</u>。这里的*约束规则*其实就是指数据的不变性。譬如 *非空*、*唯一*、*无符号*、*枚举*、*数据类型*、*引用完整性（外键）* 等等。事务在执行期间一旦违反不变性，就会透过回滚操作来恢复一致性<span style="color:gray">（如：违反唯一约束会抛出 DuplicateKeyException）</span>。所以该承诺主要依赖的是 *Undo Logs（MVCC）*。
