...
<b>内容修订</b>
- 2025/04/16：重新编辑了一致性中的例子，使其更易于阅读和理解
...
<b style="color:#2C5B66">一致性（Consistency）</b>在不同的上下文中会有不同的解释。即便在关系型数据库中，一致性也不止有一种含义。因为很多人会错误将隔离性（中的读一致问题）理解为一致性，但这是错误的（具体参考：[Consistency (database systems)](https://en.wikipedia.org/wiki/Consistency_(database_systems))）。其实应该将该承诺理解为是<u>确保数据的相关*约束规则*免遭破坏</u>。这里的*约束规则*其实就是指数据的不变性。譬如 *非空*、*唯一*、*无符号*、*枚举*、*数据类型*、*引用完整性（外键）* 等等。事务在执行期间一旦违反不变性，就会透过回滚操作来恢复一致性<span style="color:gray">（如：违反唯一约束会抛出 DuplicateKeyException）</span>。所以该承诺主要依赖的是 *Undo Logs（MVCC）*。

默认情况下，***ACID一致性*仅能确保<u>（同库）数据定义</u>的不变性，而无法确保<u>业务规则</u>的不变性**。
后者通常只能在应用代码中透过特定策略来解决。例如使用*排他锁[^4]*或*乐观锁[^5]*。

理解例子：用户 `a`、`b` 共用一个电子钱包（假设余额为100元）。他们在相互不知情的情况下，几乎同时进行消费（假设 `a` 消费了 70 元，而 `b` 消费了 50 元）。在该场景下，数据会出现**并发修改**问题，导致业务规则被破坏。例如，实际消费超出了余额。注意，“超出”不一定体现为余额小于 0，它也可以是**更新覆盖**。譬如 `a` 消费了 70 元后余额为 30 元，`b` 消费 50 元后余额为 50 元。但因为 `a` 消费稍微比 `b` 要快一些，所以最终余额就会是 50 元，而非 30 元。因为 `b` 会覆盖 `a` 的更新。之所以会发生此类问题，是因为 MVCC 执行了快照读（既读操作没有互斥）。而“消费”这一动作，在应用层其实需要多个步骤才能实现：(1)读取余额，然后(2)从余额中减去消费额度，最后(3)保存计算结果。从并发编程角度，这些操作应该具备原子性（其中包括互斥性）。常见解决方案有以下这些：
- 在读操作上施加排他锁（如：for update）
- 在写操作上施加乐观锁（如：CAS + Version）
- 在写操作上执行原子更新（如：UPDATE wallet SET balance = balance - 70  WHERE id = 1 AND balance >= 70）

从该例子中，不难发现。关系型数据库在使用层面，提供了两种方案来控制一致性：锁 和 MVCC。两者应该合理地结合使用。

<b style="color:#2C5B66">隔离性（Isolation）</b>可用于控制并发事务（计算结果）之间的可见性。承诺<u>事务在并发期间可做到相互隔离[^6]</u>。InnoDB 为事务提供了`4`种隔离性。用隔离强度由低到高排列后如下：
- `READ UNCOMMITTED` 隔离级别最低，但并发性能最好。因为不会使用快照读，所以能够读取到其他并发事务未提交的计算结果，存在*脏读*隐患。
- `READ COMMITTED` 通过应用*Undo Logs快照*能够解决*脏读*问题。但它会引出另一种隐患，*不可重复读[^7]*。因为该级别下的查询会始终读取最新版本的快照，而该版本的快照可能是其他并发事务所生成的（，所以相当于存在提交级别的脏读）。
- `REPEATABLE READ` 是 InnoDB 默认的隔离级别。能够解决*不可重复读*问题，因为该级别下的查询会始终读取（相同SQL）第一次查询所生成的版本快照。注意，InnoDB 在该隔离级别下并不会出现*幻读[^8]* 问题。因为默认会使用[next-key lock](https://dev.mysql.com/doc/refman/8.4/en/glossary.html#glos_next_key_lock)来锁定记录（行）和范围间隙。
- `SERIALIZABLE` 与 `REPEATABLE READ` 大致相同。区别在于关闭 [autocommit](https://dev.mysql.com/doc/refman/8.4/en/server-system-variables.html#sysvar_autocommit) 后，前者会自动为查询谓语（where）添加 `for share（即lock in share mode）`，从<u>一致性非锁定读</u>转为<u>一致性锁定读</u>。该级别的隔离性最好的，但性能较差。

除了 *Undo Logs* 之外，还可以使用 *锁* 来进行隔离。
...
能这样做因为 <u>InnoDB 将 *Buffer Pool[^9]* 数据落盘之前，会先将其写到 *Doublewrite Buffer*。而 *Doublewrite Buffer* 则会以大块顺序的方式写入日志文件中（，所以会非常高效）</u>。
...
[^7]: 不可重复读：重复执行相同的查询，但出现结果不一致的情况。
[^8]: 幻读：一种特殊的*不可重复读*，主要针对`insert`和`delete`操作。指<u>对前后两次相同查询的结果进行比较，出现或消失了部份记录</u>。
[^9]: Buffer Pool：用于缓存表和索引的数据（，以提高查询效率）。在专用服务器上，通常建议占 80% 的物理内存。配置参考：[Buffer Pool Configuration](https://dev.mysql.com/doc/refman/8.4/en/innodb-buffer-pool.html#innodb-buffer-pool-configuration)