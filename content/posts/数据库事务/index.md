...
理解例子：用户 `a`、`b` 共用一个电子钱包（假设余额为100元）。他们在相互不知情的情况下，几乎同时进行消费（假设 `a` 消费了 70 元，而 `b` 消费了 50 元）。在该场景下，数据会出现**并发修改**问题，导致业务规则被破坏。例如，实际消费超出了余额。注意，“超出”不一定体现为余额小于 0，它也可以是**更新覆盖**。譬如 `a` 消费了 70 元后余额为 30 元，`b` 消费 50 元后余额为 50 元。但因为 `a` 消费稍微比 `b` 要快一些，所以最终余额就会是 50 元，而非 30 元。因为 `b` 会覆盖 `a` 的更新。之所以会发生此类问题，是因为当前场景的（并发）操作不具备互斥性。因为“消费”这一动作，用程序来实现其实需要多个步骤：(1)读取余额，然后(2)从余额中减去消费额度，最后(3)保存计算结果。从并发编程角度，“先读后写”的任务应该具备原子性和（资源访问的）互斥性。其中，原子性可以由本地事务提供。而互斥性则需要在读操作上施加*排他锁（如：for update）*  或*乐观锁（如：CAS + Version）*  来解决。
